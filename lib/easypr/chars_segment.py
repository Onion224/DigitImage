...
from lib.easypr import core_func
...
class CharsSegment(object):
    ...
    def charsSegment(self, input, result):
        ...
        plateType = core_func.getPlateType(tmp, True)

        input_gray = cv2.cvtColor(input, cv2.COLOR_BGR2GRAY)

        if plateType == core_func.Color.BLUE:
            ...
            threadHoldV = core_func.ThresholdOtsu(tmp)

            _, img_threshold = cv2.threshold(input_gray, threadHoldV, 255, cv2.THRESH_BINARY)
        elif plateType == core_func.Color.YELLOW:
            w = input_gray.shape[1]
            h = input_gray.shape[0]
        ...
        elif plateType == core_func.Color.WHITE:
        ...
        if not core_func.clearLiuDingChar(img_threshold):
            return 2
