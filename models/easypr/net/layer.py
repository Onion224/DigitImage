...
class ImageLayer(Layer):
    ...
    def __init__(self, image_height, image_width, n_channels, layer_name='IMAGE'):
        with tf.compat.v1.variable_scope(layer_name):
            image = tf.compat.v1.placeholder(tf.float32, [None, image_height, image_width, n_channels], 'images')
...
class LabelLayer(Layer):
    ...
    def __init__(self, layer_name='LABEL'):
        with tf.compat.v1.variable_scope(layer_name):
            label = tf.compat.v1.placeholder(tf.int64, [None], 'labels')
...
class ConvLayer(Layer):
    ...
    def __init__(self,
                 ...
                 stride=(1, 1),
                 padding='SAME',
                 weight_init=tf.compat.v1.glorot_uniform_initializer(),
                 bias_init=tf.constant_initializer(0.0)):
        n_input_channels = prev_layer.output.get_shape().as_list()[3]

        with tf.compat.v1.variable_scope(layer_name):
            filters = tf.compat.v1.get_variable('kernel',
                                      shape=[filter_height, filter_width, n_input_channels, n_filters],
                                      initializer=weight_init)
            biases = tf.compat.v1.get_variable('biases',
                                     shape=[n_filters],
                                     initializer=bias_init)
            layer_output = tf.compat.v1.nn.bias_add(
                tf.compat.v1.nn.conv2d(input=prev_layer.output,
                             filter=filters,
...
class ActivationLayer(Layer):
    ...
    def __init__(self, prev_layer, activation=tf.compat.v1.nn.relu, layer_name='ACTIVATION'):
        with tf.compat.v1.variable_scope(layer_name):
...
class MaxPoolLayer(Layer):
    ...
    def __init__(self, prev_layer, pool_height, pool_width, layer_name='MAXPOOL', padding='SAME'):
        with tf.compat.v1.variable_scope(layer_name):
            layer_output = tf.compat.v1.nn.max_pool(prev_layer.output,
...
class DenseLayer(Layer):
    ...
    def __init__(self,
                 ...
                 weight_init=tf.compat.v1.glorot_uniform_initializer(),
                 bias_init=tf.constant_initializer(0.0)):
        n_in = prev_layer.output.get_shape()[1]

        with tf.compat.v1.variable_scope(layer_name):
            weights = tf.compat.v1.get_variable('weights', shape=[n_in, n_out], initializer=weight_init)
            biases = tf.compat.v1.get_variable('biases', shape=[n_out], initializer=bias_init)
            layer_output = tf.compat.v1.matmul(prev_layer.output, weights) + biases
...
class DropoutLayer(Layer):
    def __init__(self, prev_layer, prob, layer_name='dropout'):
        with tf.name_scope(layer_name):
            layer_output = tf.compat.v1.nn.dropout(prev_layer.output, prob)
        super().__init__(layer_in=prev_layer.output, layer_out=layer_output)
...
class ConvPoolLayer(Layer):
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope("CONVPOOL" + '_' + layer_name):
