...
class Train(object):
    ...
    def train(self, train_dataset, val_dataset):
        ...
        train_writer = tf.compat.v1.summary.FileWriter(str(self.log_dir / "summary" / "train"), self.sess.graph)
        test_writer = tf.compat.v1.summary.FileWriter(str(self.log_dir / "summary" / "test"))
        model_dir = self.log_dir / 'models'

        saver = tf.compat.v1.train.Saver(max_to_keep=3)
        tf.compat.v1.train.write_graph(self.sess.graph_def, model_dir, 'model.pb', False)
